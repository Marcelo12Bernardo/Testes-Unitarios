...
const createMenu = (menu) => {
  let newMenu = {
    fetchMenu: () => menu,
    consumption: [],
    order: (item) => newMenu.consumption.push(item),
    pay: () => {
      let intesCosumidos = newMenu.consumption;
      let totConsumido = 0;
      for (let key = 0; key < intesCosumidos.length; key += 1) {
        switch (intesCosumidos[key]) {
          case 'coxinha':
            totConsumido += 5;
          break;
          case 'agua':
